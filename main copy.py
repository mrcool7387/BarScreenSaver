...
def select_audio_device():
    """Zeigt GUI mit Dropdown zur Auswahl des Audioeingabegeräts"""
    ...
    if not input_devices:
        messagebox.showerror("Kein Gerät gefunden",
                             "Es wurden keine Aufnahmegeräte gefunden.\n"
                             "Aktiviere z. B. 'Stereo Mix' in den Soundeinstellungen.")
        return None

    selected_device = ctk.StringVar()
    selected_device.set(str(input_devices[0][0]))  # erstes Gerät vorgewählt

    root = ctk.CTk()
    root.title("Audioquelle auswählen")
    root.geometry("420x180")
    root.resizable(False, False)

    ctk.CTkLabel(root, text="Bitte wähle ein Audioeingabegerät:",
              font=("Segoe UI", 11)).pack(pady=10)

    combo = ctk.CTkComboBox(root, textvariable=selected_device,
                         values=[f"{i}: {name}" for i, name in input_devices],
                         state="readonly", font=("Segoe UI", 10))
    combo.pack(pady=5, ipadx=4, fill="x", padx=20)

    chosen = {"id": None}

    def confirm():
        val = selected_device.get().split(":")[0].strip()
        chosen["id"] = int(val)
        root.destroy()

    ctk.CTkButton(root, text="Starten", command=confirm).pack(pady=15)
...
class AudioCapture(threading.Thread):
    ...
    def audio_callback(self, indata, frames, time_info, status):
        # FFT auf den PCM-Daten
        data = np.abs(np.fft.rfft(indata[:, 0]))
        data = data[:len(data)//2]
        # Logarithmische Skalierung für menschliches Gehör
        data = np.log1p(data)
        # In BAR_COUNT Bänder aufteilen
        bands = np.array_split(data, BAR_COUNT)
        spectrum = np.array([np.mean(b) for b in bands])
        spectrum /= np.max(spectrum) + 1e-6  # Normalisieren
        self.q.put(spectrum)

    def run(self):
        with sd.InputStream(device=None, channels=1, samplerate=SAMPLE_RATE,
